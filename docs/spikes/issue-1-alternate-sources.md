...
> **结论先行**：714 只缺口里 **34 只可补**（全是 FOF），**680 只上游确实
> 没有当期数据**。Issue 里"~1000 只、货币基金占 ~900"的假设已被
> `fund-fallback-fetcher` 上线后的实测推翻。
>
> ⚠️ **本文档第 4 节最初把可补的 34 只归因于 `pingzhongdata.js` 独有 —— 这是
> 错的**。落地时做了 A/B 对照（同一批 code、同一时间窗同时测两源），结果是
> pingzhongdata 独有 **0 只**：那 34 只 lsjz 一直都能取到，只是日常 fallback
> 的回看窗口（3 天）碰不到它们的披露日。真正的修复是把窗口放宽到 14 天，
> 见第 6 节。
>
...
## 6. 落地结果：真正的修复是回看窗口，不是新数据源（2026-08-03）

按第 5 节建议实现 pingzhongdata 分支后，做了一次 A/B 对照测量 —— 对全部 714 只
缺口 code，在**同一时间窗**（2026-07-20 ~ 08-03）同时调用两个端点：

| | 只数 |
|---|---|
| lsjz 有数据 | 36 |
| pingzhongdata 有数据 | 34 |
| **仅 pingzhongdata 独有** | **0** |
| 两者都无 | 678 |

**pingzhongdata 能拿到的是 lsjz 的子集，增量为零。** 新分支已回退。

### 我在 spike 阶段的方法错误

第 3 节只测了 pingzhongdata，**没有在同一批 code、同一窗口上对照测 lsjz**，
然后把"pingzhongdata 有 34 只"直接读成了"lsjz 没有的 34 只"。缺少对照组，
就把一个共有集合误判成独有集合。

### 真实根因

那 34 只（+2 只定开债）是**持有期型产品**：3/6 个月持有的 FOF 按自己的节奏
披露净值。它们最新披露日是 2026-07-24，而日常 fallback 只回看 **3 天** ——
窗口永远碰不到披露日，于是这些基金**每天都被算作缺失**，而数据一直躺在 lsjz 里。

### 修复

`fund-fallback-fetcher` 默认回看窗口 **3 天 → 14 天**
（`FALLBACK_LOOKBACK_DAYS` 可覆盖）。14 天足以跨过季度持有型产品的披露间隔，
又不会拉太多历史拖慢运行。

实测：窗口 `2026-07-20 → 2026-08-03`，**补回 34 只 / 41 行**，缺口 714 → 680。

### 修正后的建议

1. ~~加 pingzhongdata 分支~~ → **已回退，零增量**。
2. ✅ **已做**：回看窗口放宽到 14 天，回收 34 只 FOF + 2 只定开债。
3. **待做**：freshness-check 加已知豁免名单（680 只里绝大多数是已终止产品）。
4. **不做**：付费数据源采购 —— 结论不变，680 只上游无当期数据。
5. **不做**：继续找 QDII 美元现汇替代源 —— 三源否证。

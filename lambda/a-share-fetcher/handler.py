"""Lambda handler for A-share market data fetch (dual-write: raw parquet + Iceberg)."""
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"A-share fetch completed: {len(uploads)} uploads, {len(errors)} errors "
            f"in {elapsed:.2f}s"
        )
        ...
        elapsed = (datetime.now() - start_time).total_seconds()
        logger.exception("A-share fetch failed")
        return {
            "statusCode": 500, "downloader": "a-share", "success": False,

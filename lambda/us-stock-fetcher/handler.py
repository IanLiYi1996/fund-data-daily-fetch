"""Lambda handler for US stock + US macro data fetch (dual-write: raw parquet + Iceberg)."""
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"US stock/macro fetch completed: {len(uploads)} uploads, {len(errors)} errors "
            f"in {elapsed:.2f}s"
        )
        ...
        elapsed = (datetime.now() - start_time).total_seconds()
        logger.exception("US stock/macro fetch failed")
        return {

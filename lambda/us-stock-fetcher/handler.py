"""Lambda handler for US stock + US macro data fetch."""
...
from shared.fetchers import USStockFetcher
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    logger.info(f"Starting US stock/macro data fetch. Event: {json.dumps(event, default=str)}")
    ...
    try:
        ...
        logger.info(f"US stock/macro fetch completed: {len(uploads)} uploads, {len(errors)} errors in {elapsed:.2f}s")

        return {
            "statusCode": 200, "downloader": "us-stock", "success": True,
            ...
            "catalog": USStockFetcher.get_data_catalog(),
        }
    except Exception as e:
        elapsed = (datetime.now() - start_time).total_seconds()
        logger.error(f"US stock/macro fetch failed: {e}")
        return {
            "statusCode": 500, "downloader": "us-stock", "success": False,

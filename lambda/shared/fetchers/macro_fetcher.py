...
class MacroFetcher(BaseFetcher):
    ...
    def fetch_all(self) -> FetchSummary:
        ...
        results = []
        results.append(self._safe_fetch("macro_lpr", self._fetch_lpr))
        results.append(self._safe_fetch("macro_cpi", self._fetch_cpi))
        results.append(self._safe_fetch("macro_ppi", self._fetch_ppi))

"""Lambda handler for CN macroeconomic data fetch."""
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """Fetch CN macroeconomic data and upload to S3.
    ...
    try:
        ...
        logger.info(f"CN macro fetch completed: {len(uploads)} uploads, {len(errors)} errors in {elapsed:.2f}s")
        ...
        elapsed = (datetime.now() - start_time).total_seconds()
        logger.error(f"CN macro fetch failed: {e}")
        return {
            "statusCode": 500,
            "downloader": "cn-macro",
            "success": False,
